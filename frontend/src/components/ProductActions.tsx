...
export default function ProductActions({ product }: ProductActionsProps) {
  const { addToCart, setCartForCheckout, restoreCart } = useCart();
  ...
  const handleBuyNow = () => {
    setCartForCheckout(product);
    setIsModalOpen(true);
  };

  const handleCloseModal = () => {
    setIsModalOpen(false);
    restoreCart(); // Restore the original cart if checkout is cancelled
  };

  const handleOrderConfirmed = () => {
    setIsModalOpen(false);
    // The cart is already cleared by the checkout form, so no need to restore
  }
  ...
  return (
    <>
      <div className="flex items-center justify-between space-x-2">
        <button onClick={handleBuyNow} className="flex-1 h-11 bg-black text-white text-sm font-bold rounded-lg hover:scale-105 transition-transform">Buy Now</button>
        <button onClick={handleAddToCart} className="flex-1 h-11 bg-[#D9D9D9] text-[#2E2E2E] text-sm font-bold rounded-lg hover:scale-105 transition-transform">Add to Cart</button>
        <button onClick={handleWhatsAppOrder} className="flex-1 h-11 bg-[#25D366] text-white text-sm font-bold rounded-lg flex items-center justify-center space-x-2 hover:scale-105 transition-transform">
          <WhatsAppIcon />
          <span>WhatsApp</span>
        </button>
      </div>
